...
def evaluate_a_pair(data_indices, pair_name):
    ...
    with codecs.open('./model/actions.csv', mode='w', encoding='utf-8') as f: #Update where you want to save
        f.write('\n'.join(str(action_values) for action_values in saved_a))
...
def plot_rs_dist_overlap(rs, fig_title, mean=False, median=False, color=None, methodName=None):
    ...
    if mean:
        plt.axvline(mean_stat, color=color, linestyle='dashed', linewidth=1,
                    label=methodName + ' Mean: {:.3f}'.format(mean_stat))
    if median:
        plt.axvline(median_stat, color=color, linestyle='dashed', linewidth=1,
                    label=methodName + ' Median: {:.3f}'.format(median_stat))
...
def train(data_indices, num_of_batch):
    ...
    _logger.info('num_of_batch = {}, estimated epoch = {}'.format(num_of_batch,
                                                                  num_of_batch * batch_size / 2 / rl_constants.num_of_pair))
...
def generate_parser():
    ...
    parser.add_argument("--rand_action_prob", type=float, default=0.0,
                        help="random action probability lower is better...")
...
def write_response(train_total_r_dict):
    csv_to_write = ''
    for key in train_total_r_dict:
        csv_to_write += str(key) + ',' + str(train_total_r_dict[key]) + '\n'

    with codecs.open('./model/output.csv', mode='w', encoding='utf-8') as f: #Update where you want to save
...
def plot_distribution(config):
    ...
    # evaluate performance on test dataset
    test_rs, test_total_r_dict = run_epoch_for_evaluate_performance(config.test_indices)
    write_response(test_total_r_dict)
    _logger.info("\n{}".format(test_rs))
